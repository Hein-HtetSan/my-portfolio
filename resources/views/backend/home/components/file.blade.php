...
<div class="w-full h-dvh bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
    <form action="{{ route('profile.cv.save') }}" class="w-4/12 bg-white dark:bg-gray-500 p-5 rounded-lg shadow-xl" method="POST"
    enctype="multipart/form-data">
        ...
        <div class="mb-4  flex flex-col gap-3 items-start justify-center">
            ...
            {{-- input  --}}
            <label for="cv_form" class="text-lg font-rubik text-slate-700 dark:text-slate-300">Insert File</label>
            <input type="file" class="w-full rounded-lg text-slate-600 dark:text-slate-200 p-3
            bg-slate-200 dark:bg-slate-600 outline-sky-600" name="cv_form" value="{{ old('cv_form', $cv->cv_form) }}">
            @error('cv_form')

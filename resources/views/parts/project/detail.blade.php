...
@section('content')
    ...
    <section class="flex items-center justify-center w-full h-auto py-8 md:py-20 mt-6 md:mt-20">
        ...
        <main class="bg-white dark:bg-slate-900 p-5 rounded-lg w-full md:w-3/4 shadow-xl">
            ...
            {{-- back button  --}}
            <a href="{{ route('user.works') }}" class="text-slate-400 font-rubik hover:text-sky-400"> <i
                    class="bx bx-arrow-left"></i> Back</a>
            ...
            @if (count($project['covers']) == 3)
                <div class="grid grid-cols-12 flex flex-col md:flex-row gap-2 mb-5">
                    <img src="{{ asset('storage/' . $project['covers'][0]->name) }}" alt=""
                        class="col-span-12 rounded-lg" style="width: 100%; height: 40vh;">
                    <img src="{{ asset('storage/' . $project['covers'][1]->name) }}" alt=""
                        class="col-span-12 md:col-span-6 rounded-lg" style="width: 100%; height: 40vh;">
                    <img src="{{ asset('storage/' . $project['covers'][2]->name) }}" alt=""
                        class="col-span-12 md:col-span-6 rounded-lg" style="width: 100%; height: 40vh;">
                </div>
            ...
            <div class="flex items-center justify-start gap-2">
                <div class="flex flex-col gap-3 ">
                    <div class="flex items-center justify-start gap-3">
                        <span class="text-slate-500 font-rubik uppercase">Get the source code here: </span><a
                            href="{{ $project->github }}"
                            class="px-4 py-2 text-slate-300 bg-slate-700 font-rubik rounded-lg "> <i
                                class="bx bxl-github"></i> GitHub</a>
                    </div>
                    @if ($project->demo)
                        <div class="flex items-center justify-start gap-3">
                            <span class="text-slate-500 font-rubik uppercase">Check Demo: </span><a
                                href="{{ $project->demo }}" class="text-sky-400 dark:text-sky-500"> <i
                                    class="bx bx-play"></i> Demo</a>
                        </div>
                    @endif
                    {{-- <div class="flex items-center justify-start gap-3">
                        <!-- Facebook Share Button -->
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ $currentUrl }}"
                            class="px-4 py-2 text-slate-500 dark:text-slate-300 bg-slate-200 rounded-lg dark:bg-slate-700 font-rubik"
                            target="_blank">
                        ...
                        <a href="mailto:?subject=Check out this project&body={{ $currentUrl }}"
                            class="px-4 py-2 text-slate-500 dark:text-slate-300 bg-slate-200 rounded-lg dark:bg-slate-700 font-rubik">
                            <i class="bx bx-envelope"></i>
                        </a>
                        <a href="https://x.com/intent/tweet?url={{ $currentUrl }}&text={{ urlencode($project->name) }}"
                            class="px-4 py-2 text-slate-500 dark:text-slate-300 bg-slate-200 rounded-lg dark:bg-slate-700 font-rubik"
                            target="_blank">
